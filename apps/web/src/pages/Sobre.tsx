  ...
  return (
    <main className="min-h-screen bg-background">
      <section className="relative overflow-hidden border-b border-line pb-16 pt-32 sm:pb-20 sm:pt-36">
        <div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_30%_0%,rgba(166,111,24,0.14),transparent_44%)]" aria-hidden="true" />
        <div className="relative mx-auto grid max-w-7xl gap-12 px-4 sm:px-6 lg:grid-cols-[1.1fr_.9fr] lg:items-center">
          <div>
            ...
            <Reveal delay={0.08}>
              <h1 className="mt-4 max-w-4xl font-serif text-4xl font-bold leading-[1.04] text-text-primary sm:text-5xl md:text-6xl">
          ...
          <Reveal delay={0.2}>
            <aside className="rounded-3xl border border-text-primary/[0.08] bg-surface-2 p-6 sm:p-8" aria-label="Responsável pelo Rei das Vendas">
              <div className="flex items-center gap-4">
                <span className="flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-gold-light to-gold-dark font-serif text-2xl font-bold text-[#FFFDF8]" aria-hidden="true">TP</span>
                <div>
                  <h2 className="font-serif text-2xl font-bold text-text-primary">{BRAND.founder.name}</h2>
                  <p className="mt-1 text-sm text-gold">Direção de projeto e estratégia digital</p>
                </div>
              </div>
              <p className="mt-6 text-sm leading-7 text-text-secondary">{BRAND.founder.bio}</p>
              <div className="mt-6 flex flex-wrap gap-3 text-xs text-text-secondary">
                <span className="inline-flex items-center gap-2 rounded-full border border-text-primary/10 px-3 py-2"><MapPin className="h-3.5 w-3.5 text-gold" /> Franca, SP</span>
                              </div>
            </aside>
          </Reveal>
        </div>
      ...
      <section className="py-16 sm:py-24">
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          <div className="max-w-3xl">
            ...
            <h2 className="mt-4 font-serif text-3xl font-bold text-text-primary sm:text-5xl">Autoridade demonstrada por organização, clareza e acabamento.</h2>
          </div>
          <div className="mt-10 grid gap-5 md:grid-cols-2">
            {PRINCIPLES.map((principle) => {
              const Icon = principle.icon;
              return (
                <article key={principle.title} className="rounded-3xl border border-text-primary/[0.08] bg-surface-2 p-6 sm:p-8">
                  ...
                  <h3 className="mt-5 font-serif text-2xl font-bold text-text-primary">{principle.title}</h3>
      ...
      <section className="border-y border-line bg-surface py-16 sm:py-24">
        <div className="mx-auto grid max-w-7xl gap-10 px-4 sm:px-6 lg:grid-cols-2 lg:items-center">
          <div>
            ...
            <h2 className="mt-4 font-serif text-3xl font-bold text-text-primary sm:text-5xl">O cliente não precisa coordenar vários fornecedores para colocar o site no ar.</h2>
            <p className="mt-5 text-base leading-7 text-text-secondary">Planejamento, construção, revisão e publicação são conduzidos de forma integrada, com os limites de cada projeto descritos antes do início.</p>
          </div>
          <ul className="grid gap-3 rounded-3xl border border-text-primary/[0.08] bg-surface-3 p-6 sm:p-8">
      ...
      <section className="py-16 sm:py-24">
        <div className="mx-auto max-w-3xl px-4 text-center sm:px-6">
          <h2 className="font-serif text-3xl font-bold text-text-primary sm:text-5xl">Sua empresa já tem valor. O site precisa deixar isso evidente.</h2>
          <p className="mx-auto mt-5 max-w-2xl text-base leading-7 text-text-secondary">Envie o site atual ou o perfil da empresa no Google para receber uma leitura inicial dos pontos que merecem prioridade.</p>
