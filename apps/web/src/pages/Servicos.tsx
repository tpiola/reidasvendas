    ...
    <main className="min-h-screen bg-background">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          ...
          >
            {DELIVERABLES.map((item) => {
              ...
              return (
                <motion.article key={item.title} variants={staggerItem} className="rounded-3xl border border-text-primary/[0.08] bg-surface-2 p-6">
                  ...
                  <h3 className="mt-5 font-serif text-2xl font-bold text-text-primary">{item.title}</h3>
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          <div className="grid gap-12 lg:grid-cols-[.8fr_1.2fr] lg:items-start">
            <div>
              ...
              <h2 className="mt-4 font-serif text-3xl font-bold text-text-primary sm:text-5xl">Um processo simples, conduzido com cuidado.</h2>
              <p className="mt-5 max-w-xl text-base leading-7 text-text-secondary">O escopo é definido depois da análise, para recomendar somente as páginas e conexões que realmente fazem sentido.</p>
            </div>
            <div className="border-t border-text-primary/10">
              {PROCESS.map(([number, title, text]) => (
                <article key={number} className="grid gap-3 border-b border-text-primary/10 py-6 sm:grid-cols-[52px_190px_1fr]">
      ...
      <section className="py-16 sm:py-24">
        <div className="mx-auto grid max-w-7xl gap-10 px-4 sm:px-6 lg:grid-cols-2 lg:items-center">
          <div>
            ...
            <h2 className="mt-4 font-serif text-3xl font-bold text-text-primary sm:text-5xl">Sem pacotes inflados e sem promessas genéricas.</h2>
            <p className="mt-5 text-base leading-7 text-text-secondary">O investimento depende da quantidade de páginas, conteúdo, integrações e necessidades do negócio. Tudo é descrito na proposta antes do início.</p>
          </div>
      ...
      <section className="border-t border-line bg-surface-2 py-16 sm:py-24">
        <div className="mx-auto max-w-3xl px-4 text-center sm:px-6">
          <ClipboardCheck className="mx-auto h-9 w-9 text-gold" aria-hidden="true" />
