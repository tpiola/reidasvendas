    ...
    <main className="min-h-screen bg-[#030303]">
      <section className="relative overflow-hidden border-b border-white/[0.06] pb-16 pt-32 sm:pb-20 sm:pt-36">
        ...
        <div className="relative mx-auto max-w-4xl px-4 text-center sm:px-6">
          ...
          <Reveal delay={0.08}>
            <h1 className="mt-4 font-serif text-4xl font-bold leading-[1.04] text-white sm:text-5xl md:text-6xl">
      ...
      <section className="py-16 sm:py-24">
        <div className="mx-auto grid max-w-7xl gap-10 px-4 sm:px-6 lg:grid-cols-2 lg:items-center">
          <div>
            ...
            <h2 className="mt-4 font-serif text-3xl font-bold text-white sm:text-5xl">Cada empresa exige uma combinação diferente de páginas e recursos.</h2>
            <p className="mt-5 text-base leading-7 text-[#A1A1AA]">A proposta separa o que está incluído, o que depende do cliente e quais itens são opcionais. Nenhum valor ou condição é publicado antes de ser confirmado.</p>
          </div>
          <ul className="grid gap-3 rounded-3xl border border-white/[0.08] bg-[#090909] p-6 sm:p-8">
            {FACTORS.map((factor) => (
              <li key={factor} className="flex gap-3 text-sm leading-6 text-[#D4D4D8]">
      ...
      <section className="border-y border-white/[0.06] bg-[#080808] py-16 sm:py-24">
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          <div className="max-w-3xl">
            ...
            <h2 className="mt-4 font-serif text-3xl font-bold text-white sm:text-5xl">Três passos antes de iniciar.</h2>
          </div>
          <div className="mt-10 grid gap-5 lg:grid-cols-3">
            {STEPS.map(([number, title, text]) => (
              <article key={number} className="rounded-3xl border border-white/[0.08] bg-[#0D0D0D] p-6 sm:p-8">
                <span className="text-xs font-bold text-[#D6A84F]">0{number}</span>
      ...
      <section className="py-16 sm:py-24">
        <div className="mx-auto max-w-3xl px-4 text-center sm:px-6">
          <h2 className="font-serif text-3xl font-bold text-white sm:text-5xl">Comece pela análise, não pela escolha de um pacote.</h2>
          <p className="mx-auto mt-5 max-w-2xl text-base leading-7 text-[#A1A1AA]">Envie o site atual, o perfil da empresa no Google ou uma descrição do negócio. A recomendação será baseada no que precisa ser corrigido e construído.</p>
          <div className="mt-8">
            <PremiumButton href="/contato">Quero solicitar uma análise <ArrowRight className="h-4 w-4" /></PremiumButton>
          </div>
          <p className="mt-5 text-xs leading-5 text-[#8A8A92]">A análise inicial não representa garantia de posicionamento, vendas ou resultados específicos.</p>
        </div>
      </section>
    </main>
  );
