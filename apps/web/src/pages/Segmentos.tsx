    ...
    <main>
      ...
      <section className="relative overflow-hidden pt-28 pb-16 sm:pt-32 sm:pb-20">
        ...
        <div className="relative mx-auto max-w-7xl px-4 text-center sm:px-6">
          ...
          <Reveal delay={0.1}>
            <h1 className="mt-6 font-serif text-4xl font-bold leading-tight text-text-primary sm:text-5xl md:text-6xl">
          ...
          <Reveal delay={0.25}>
            <div className="mt-6 flex flex-wrap justify-center gap-3 text-sm text-text-muted">
              <span className="flex items-center gap-1.5">
                <CheckCircle2 className="h-4 w-4 text-gold" /> Soluções Sob Medida
              </span>
              <span className="hidden sm:inline text-[rgba(28,25,20,0.12)]">•</span>
              <span className="flex items-center gap-1.5">
                <Star className="h-4 w-4 text-gold" /> Cases de Sucesso
              </span>
              <span className="hidden sm:inline text-[rgba(28,25,20,0.12)]">•</span>
              <span className="flex items-center gap-1.5">
                <Zap className="h-4 w-4 text-gold" /> Entrega Rápida
              </span>
            </div>
          </Reveal>
        </div>
                    ...
                    return (
                      <button
                        ...
                        className={`flex w-full items-center gap-2.5 rounded-xl px-4 py-2.5 text-left text-xs font-semibold transition-all duration-300 ${
                          filtro === cat.id
                            ? 'bg-[rgba(166,111,24,0.1)] text-gold border border-[rgba(166,111,24,0.2)]'
                            : 'text-text-secondary hover:bg-text-primary/[0.03] hover:text-text-primary border border-transparent'
                        }`}
                      >
                    ...
                    return (
                      <button
                        ...
                        className={`inline-flex items-center gap-1.5 rounded-full px-3.5 py-1.5 text-[10px] font-semibold uppercase tracking-[0.12em] transition-all duration-300 ${
                          filtro === cat.id
                            ? 'bg-gold text-[#FFFDF8]'
                            : 'border border-[rgba(28,25,20,0.1)] text-text-secondary hover:border-[rgba(166,111,24,0.3)] hover:text-text-primary'
                        }`}
                      >
            ...
            <div className="flex-1">
                ...
                {segmentosFiltrados.map((segmento) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Title & Desc */}
                      <h3 className="font-serif text-lg font-bold text-text-primary">{segmento.title}</h3>
                      <p className="mt-2 text-sm leading-relaxed text-text-secondary">{segmento.desc}</p>
              ...
              {segmentosFiltrados.length === 0 && (
                <div className="py-20 text-center">
                  <p className="text-sm text-text-muted">Nenhum segmento encontrado com este filtro.</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="relative mx-auto max-w-3xl px-4 text-center sm:px-6">
          ...
          <Reveal delay={0.2}>
            <div className="mt-8 flex flex-wrap items-center justify-center gap-x-6 gap-y-3 text-xs text-text-muted">
              <span className="flex items-center gap-1.5">
                <Globe className="h-3.5 w-3.5 text-gold" /> Atendemos em Todo o Brasil
              </span>
              <span className="flex items-center gap-1.5">
                <Shield className="h-3.5 w-3.5 text-gold" /> Diagnóstico Sem Compromisso
              </span>
              <span className="flex items-center gap-1.5">
                <Zap className="h-3.5 w-3.5 text-gold" /> Projeto em Até 7 Dias
              </span>
            </div>
          </Reveal>
        </div>
